...
@section('content')
@if(Session::has('ingret')) <div class="alert alert-success"> {{Session::get('ingret')}} </div> @endif

@if(Session::has('ingadded')) <div class="alert alert-success"> {{Session::get('ingadded')}} </div> @endif
        ...
        <div class="col-md-10">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <h3>Products Shopping Report: {{$date}}</h3>
                    <h5 style="color:#363636">Balance Start Date: {{$bsdatex}}</h5>
                    <hr>
                    <form method="POST" action="/productsReport">
                        @csrf
                                                    ...
                                                    @if($k==0)
                                                        @foreach($invx as $in)
                                                            @if($in->name==$investorx[$i]&&$in->date==$datesx[$j])
                                                                @if($pos==0)
                                                                    {{$in->price}} <span style="color:#ef820d;">({{$in->qty}} {{$in->unit_name}})</span>
                                                                    @php
                                                                    $profit+=$in->price;
                                                                    @endphp
                                                                @endif
