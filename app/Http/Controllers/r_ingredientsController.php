...
class r_ingredientsController extends Controller
{
    /**
    ...
    public function index()
    {
        if (Gate::allows('manager')) 
            ...
            return view('/r_ingredients',['menulist'=>$menulist,'category'=>$category]);
         ...
         $c=0;
            ...
            foreach($ingred as $ing)
            {
                ...
                if($rida!=NULL)
                {
                    ...
                    if($ridb==NULL)
                        return \Redirect::route('r_ingredients.index')
                            ->with('rcombo', 'Please Enter Both Qty & Total Price for All Ingredients!');
                    else
                    {
                        $inv_amt=ingredients::where('f_id','=',$ing->id)->get();
                        ...
                        $rinv_amt=r_ingredients::where('f_id','=',$ing->id)->get();
            ...
            if($c!=1)
                return \Redirect::route('r_ingredients.index')->with('rcombo', 'No Input Values!');
            else
            {
                foreach($ingred as $ing)
                {
                    ...
                    if($rida!=NULL)
                    {
                        ...
                        if($ridb!=NULL)
                        {
                            ...
                            $cu_price=r_ingredients::max('cu_price');
                            $cu_price+=$ridb;
                            $ingredients=new r_ingredients;
                ...
                return \Redirect::route('ingredientsReport.index')->with('ingret','Ingredients Return Successfully Entered!');
