...
class productsReportController extends Controller
{
    ...
    public function index()
    {
        if (Gate::allows('manager')) 
            ...
            $inv = products::whereDate('date',$dt)
            ->join('menu', 'products.f_id', '=', 'menu.id')
            ->join('units','menu.f_id','=','units.id')
            ->select('menu.name','products.qty', 'products.date','products.price','units.unit_name')
            ->get();
            $rinv = r_products::whereDate('date',$dt)
            ->join('menu', 'r_products.f_id', '=', 'menu.id')
            ->join('units','menu.f_id','=','units.id')
            ->select('menu.name','r_products.qty', 'r_products.date','r_products.price','units.unit_name')
            ->get();
            ...
            return view('/productsReport',['invx' => $inv,'rinvx' => $rinv,'investorx'=>$investor,'datesx' => $dates,'fdatex'=>$fdate,'date'=>$dt,'bsdatex'=>$bsdate]);
    ...
    public function store(Request $request)
    {
        if (Gate::allows('manager')) 
        {
            ...
            $inv = products::whereDate('date','>=',$from)
            ->whereDate('date','<=',$to)
            ->join('menu', 'products.f_id', '=', 'menu.id')
            ->join('units','menu.f_id','=','units.id')
            ->select('menu.name','products.qty', 'products.date','products.price','units.unit_name')
            ->latest('date')
            ->get();
            $rinv = r_products::whereDate('date','>=',$from)
            ->whereDate('date','<=',$to)
            ->join('menu', 'r_products.f_id', '=', 'menu.id')
            ->join('units','menu.f_id','=','units.id')
            ->select('menu.name','r_products.qty', 'r_products.date','r_products.price','units.unit_name')
            ...
            return view('/productsReport',['invx' => $inv,'rinvx' => $rinv,'investorx'=>$investor,'datesx' => $dates,'fdatex'=>$fdate,'date'=>$dt,'bsdatex'=>$bsdate]);
                    ...
                    return \Redirect::route('productsReport.index')        
