...
class cin_othersReportController extends Controller
{
    /**
    ...
    public function index()
    {
        if (Gate::allows('manager')) 
            ...
            $inv = cin_others::whereDate('date',$dt)
            ->join('cin_list', 'cin_others.f_id', '=', 'cin_list.id')
            ->select('cin_list.name', 'cin_others.date', 'cin_others.amt')
            ->get();
            $rinv = r_cin_others::whereDate('date',$dt)
            ->join('cin_list', 'r_cin_others.f_id', '=', 'cin_list.id')
            ->select('cin_list.name', 'r_cin_others.date', 'r_cin_others.amt')
            ->get();
            ...
            return view('/cin_othersReport',['invx' => $inv,'rinvx' => $rinv,'investorx'=>$investor,'datesx' => $dates,'fdatex'=>$fdate,'date'=>$dt,'bsdatex'=>$bsdate]);
    ...
    public function store(Request $request)
    {
        if (Gate::allows('manager')) 
        {
            ...
            {
            $dt='From( '.$from.' ) To( '.$to.' )';
            $inv = cin_others::whereDate('date','>=',$from)
            ->whereDate('date','<=',$to)
            ->join('cin_list', 'cin_others.f_id', '=', 'cin_list.id')
            ->select('cin_list.name', 'cin_others.date', 'cin_others.amt')
            ->latest('date')
            ->get();
            $rinv = r_cin_others::whereDate('date','>=',$from)
            ->whereDate('date','<=',$to)
            ->join('cin_list', 'r_cin_others.f_id', '=', 'cin_list.id')
            ->select('cin_list.name', 'r_cin_others.date', 'r_cin_others.amt')
            ->latest('date')
